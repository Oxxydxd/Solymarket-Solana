...
app.get('/api/orderbook/:marketId', async (req, res) => {
  try {
    ...
    // Fetch market options and metadata for mapping
    ...
    // Parse metadata
    const metadata = typeof marketResult.rows[0].metadata === 'string'
      ? JSON.parse(marketResult.rows[0].metadata)
      : (marketResult.rows[0].metadata || {});
    ...
    // Return metadata along with orderBook
...
app.get('/api/markets', async (req, res) => {
  try {
    ...
    const markets = result.rows.map(row => ({
      ...
      // Ensure metadata is parsed as JSON
...
app.get('/api/markets/:id', async (req, res) => {
  try {
    ...
    // Ensure metadata is parsed as JSON
    market.metadata = typeof market.metadata === 'string' ? JSON.parse(market.metadata) : (market.metadata || {});
  ...
  try {
    ...
    if (isAdmin) {
      ...
      `, [
        ...
        'under_review',
        JSON.stringify({ marketImage: req.body.marketImage || null })
      ]);
    } else {
...
app.get('/api/bets', async (req, res) => {
  try {
    ...
    // Accept both market_id and marketId
...
// Serve static frontend from /public (after all API routes)
app.use(express.static(path.join(__dirname, 'public')));

// Handle favicon.ico requests to avoid errors
app.get('/favicon.ico', (req, res) => res.status(204).end());

// Fallback to index.html for SPA (after all API routes and static)
app.get('*', (req, res) => {
